...
# RoxMux_fela (FELA fork of RoxMux 1.7.3)

Fork of [neroroxxx/RoxMux](https://github.com/neroroxxx/RoxMux) **1.7.3** for DCO Input:

* `Rox74HC40XX`: `micros()` scan interval + analog/digital `update(..., digitalOrAnalog)`
* `Rox74HC165` / `Rox74HC40XX` / `RoxSerialMonitor`: `Serial` calls commented out (RP2040 USB)
* Arduino name / umbrella: `#include <RoxMux_fela.h>`
...
RoxMux contains multiple libraries in one, just include `<RoxMux_fela.h>` to get started. All the libraries are templated to reduce RAM usage.
